...
const beverageFamilies = ["Vino", "Ron", "Whisky", "Ginebra", "Tequila"] as const;
...
export function BeveragesCatalog({ categories }: Props) {
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 sm:grid sm:grid-cols-2 sm:gap-3 sm:overflow-visible lg:grid-cols-5">
        {beverageFamilies.map((family) => (
          <button
            key={family}
            onClick={() => setSelected((prev) => (prev === family ? null : family))}
            className={`min-w-[128px] rounded-xl border px-3 py-3 text-left transition sm:min-w-0 sm:px-4 sm:py-4 ${
              selected === family
          ...
          >
            <p className="section-title text-xl sm:text-2xl">{family}</p>
          </button>
        ))}
      </div>
