...
export async function PATCH(
  ...
  if (status === "cancelada" && previousStatus === "pendiente") {
    updates.rejection_reason = rejectionReason ? rejectionReason : null;
  }
  ...
  if (
    error &&
    /rejection_reason|schema cache/i.test(error.message) &&
    "rejection_reason" in updates
  ) {
    const { rejection_reason: _omit, ...withoutReason } = updates;
    ({ error } = await supabase
      .from("reservations")
      .update(withoutReason as never)
      .eq("id", params.id));
  }
