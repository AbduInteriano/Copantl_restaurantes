...
export function buildReservationRejectionEmail(p: ReservationEmailTemplateParams): {
  ...
  const reasonHtmlBlock = reason
    ? `
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:14px;font-size:14px;border:1px solid #fcd9b6;border-radius:8px;background:#fff7ed;" role="presentation">
      <tr><td style="padding:12px 16px;border-bottom:1px solid #fcd9b6;font-size:13px;font-weight:700;color:#9a3412;">Motivo proporcionado</td></tr>
      <tr><td style="padding:12px 16px;color:#7c2d12;line-height:1.55;white-space:pre-line;">${escapeHtml(reason)}</td></tr>
    </table>`
    : "";
